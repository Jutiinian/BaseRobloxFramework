...
function findDir(basePath, lowerName) {
  ...
  try {
    const entry = fs.readdirSync(basePath, { withFileTypes: true }).find(
...
const BLACKLISTED_DIRS = [startupDir, sharedDir, uiDir, assetsDir].reduce(
  (acc, name) => {
...
if (sharedDir)
  replicatedStorage.Shared = { $className: "Folder", $path: `src/${sharedDir}` };
...
if (uiDir)
  replicatedStorage.UI = { $className: "Folder", $path: `src/${uiDir}` };
if (pathExists("Packages"))
...
const hasStarterScripts =
  Object.keys(starterPlayerScripts).filter((k) => k !== "$className").length > 0;
...
function scanModuleAssets(moduleRootDir, replicatedRoot, storageRoot) {
  ...
  let moduleDirs;
  try {
    moduleDirs = fs.readdirSync(moduleRootDir, { withFileTypes: true })
...
if (hasFeatures) {
  scanModuleAssets(
    path.join(BASE_PATH, featuresDir),
    replicatedFeatures,
    storageFeatures
  );
}
if (hasServices) {
  scanModuleAssets(
    path.join(BASE_PATH, servicesDir),
    replicatedServices,
    storageServices
  );
}
